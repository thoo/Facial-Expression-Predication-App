...
from lib.tensorflow_model import session, y_pred, x
...
app.layout = html.Div([
    html.H2(children='Facial Expression Recognition', style=my_h2_style),
    # html.Hr(),
    dcc.Upload(
        id='upload-image',
...
def layout_plot(num, img, predictions):
    ...
    img = cv2.resize(
        img, (48 * 8, 48 * 8),
        interpolation=cv2.INTER_LINEAR)  # rescale(img,8,anti_aliasing=False)
